...
class Net2NetTransformer(pl.LightningModule):
    ...
    def get_rough_attn_map(self, batch, batch_idx, z_indices=None, c_indices=None, resized_mask_tensor=None):
        ...
        if batch is not None:  # used during training
            ...
            x = self.get_input( self.first_stage_key + '_%d' % low_size_idx, batch)
            c = self.get_input( self.cond_stage_key +'_%d' % low_size_idx, batch)
            xh = x.shape[2]
            xw = x.shape[3]
    ...
    def shared_step_SGA(self, batch, batch_idx):
        ...
        x = self.get_input( self.first_stage_key + '_%d' % high_size_idx, batch)
        c = self.get_input( self.cond_stage_key +'_%d' % high_size_idx, batch)
        xh = x.shape[2]
        xw = x.shape[3]
    ...
    def autoregressive_sample_fast256(self, z_indices, c_indices, H, W, latent_mask, batch_size=None,
        ...
        if self.is_SGA:
            ...
            the_transformer = self.transformer  # guiding transformer
