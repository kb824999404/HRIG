...
def convert_srgb_to_linear(c_srgb):
    c_linear = np.where(c_srgb <= 0.04045, c_srgb / 12.92, ((c_srgb + 0.055) / 1.055) ** 2.4)
    return c_linear

def convert_linear_to_srgb(c_linear):     
...
def particles_path_pkl(path, weather):
    return os.path.join(path, weather["weather"], "{}mm".format(weather["fallrate"]),  '*_frames.pkl')